...
class bits_vs : public bits_base<SZ>
{
	...
	{
		for (int i=bits_base<SZ>::SIZE; i<bits_base<SZ>::ARRAY_SIZE*bits_base<SZ>::BITS_INT_SIZE; i++)
		{
			this->mV[i>>bits_base<SZ>::BITS_SHIFT] &= ~(1<<(i&bits_base<SZ>::BITS_AND));
		}
	}
...
public:
	...
	{
		mem::cpy(this->mV, B.mV,bits_base<SZ>::BYTE_SIZE);
	}
	...
	bool		empty() const
	{
		for (int i=0; i<bits_base<SZ>::ARRAY_SIZE; i++)
		{
			if (this->mV[i])
			{
				return false;
	...
	bool		get_bit(const int i) const
	{
		...
		//--------------------------------------------
		assert(i>=0 && i < bits_base<SZ>::SIZE);
		return ( (this->mV[i>>bits_base<SZ>::BITS_SHIFT] & (1<<(i&bits_base<SZ>::BITS_AND)))!=0 );
	}
	...
	{
		return (mem::eql(this->mV, B.mV,bits_base<SZ>::BYTE_SIZE));
	}
